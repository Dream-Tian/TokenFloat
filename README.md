...
- 系统 Token，用于访问 `/api/log/self`
- 可选用户 ID，需要兼容部分 NewAPI 部署的 `New-Api-User` 请求头时再填写

读取范围默认覆盖本月和上月；自定义日期范围需要更早数据时，会按需从指定日期重新拉取。

## 数据来源

TokenFloat 调用 NewAPI 的个人消费数据接口 `/api/data/self` 读取消费记录，并使用返回的 Token、次数、模型和 `quota` 汇总展示。

压缩缓存保存在 `%LOCALAPPDATA%\TokenFloat\usage-index-v7.json.gz`，只包含统计周期、模型、Token、次数和 quota，不保存提示词或响应正文。
